...
function connect(socket, data) {
  ...
  // save the client to the hash object for quick access,
  // we can save this data on the socket with 'socket.set(key, value)'
  // but the only way to pull it back will be async
  chatClients[socket.id] = data;

  // now the client objtec is ready, update the client
  socket.emit('ready', { clientId: data.clientId });
  
  // auto subscribe the client to the 'public'
  subscribe(socket, { room: 'public' });
